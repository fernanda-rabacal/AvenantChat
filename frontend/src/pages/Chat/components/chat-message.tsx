...
export function ChatMessageItem({ message, shouldGroup, isLastMessage, messagesEndRef }: ChatMessageItemProps) {
  const isSystem = message.user?.name === 'System';

  return (
    <div key={message.id_message} ref={isLastMessage ? messagesEndRef : null}
      className={cn(
        "group flex",
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className={cn(
          "text-sm",
          isSystem ? "text-center" : 'mt-0.5 break-words'
          )}>{message.content}</div>
      </div>
    </div>
  );
